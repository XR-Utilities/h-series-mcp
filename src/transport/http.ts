...
export async function runHttp(port: number): Promise<void> {
  ...
  app.get("/", (_req, res) => {
    res.json({
      ...
      description:
        "MCP server for the H-Series product family (H-Index registry, H-Seal receipts). Connect any MCP client to /mcp.",
  ...
  const manifest = () => ({
    ...
    description:
      ...
      "(H-Index registry, H-Seal receipts) as " + ALL_TOOLS.length + " callable tools " +
  ...
  app.get("/llms.txt", (_req, res) => {
    ...
    const body = [
      ...
      "",
      "Model Context Protocol server exposing the H-Series product family",
      `(H-Index registry, H-Seal receipts) as ${ALL_TOOLS.length} callable tools for AI agents.`,
      ...
      "via the x402 facilitator. Accepts USDC, HBAR, ETH, XRP, SOL, RLUSD across",
      "multiple chains.",
      "",
      "## Discovery",
