...
export async function runHttp(port: number): Promise<void> {
  ...
  app.get("/", (_req, res) => {
    res.json({
      ...
      description:
        "MCP server for the H-Series product family (H-Index, H-Seal, H-Grant, H-Relay, H-Scope, H-Pact, H-Gate, H-Cert). Connect any MCP client to /mcp.",
  ...
  const manifest = () => ({
    ...
    description:
      ...
      "(H-Index, H-Seal, H-Grant, H-Relay, H-Scope, H-Pact, H-Gate, H-Cert) as " + ALL_TOOLS.length + " callable tools " +
  ...
  app.get("/llms.txt", (_req, res) => {
    ...
    const body = [
      ...
      "",
      "Model Context Protocol server exposing the H-Series product family",
      `(H-Index, H-Seal, H-Grant, H-Relay, H-Scope, H-Pact, H-Gate, H-Cert) as ${ALL_TOOLS.length} callable tools for AI agents.`,
      ...
      "via the x402 facilitator. Accepts USDC (Hedera/Base/Solana/Stellar) and",
      "RLUSD (XRPL); stablecoin-only.",
      "",
      "## Discovery",
